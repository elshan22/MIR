...
class Index:
    ...
    def index_genres(self):
        ...
        current_index = defaultdict(lambda: defaultdict(int))
        for doc in self.preprocessed_documents:
            for genre in doc[Indexes.GENRES.value]:
    ...
    def add_document_to_index(self, document: dict):
        ...
        for star in document[Indexes.STARS.value]:
            for term in star.split():
                self.index[Indexes.STARS.value][term][document['id']] += 1
        ...
        for summary in document[Indexes.SUMMARIES.value]:
            for term in summary.split():
                self.index[Indexes.SUMMARIES.value][term][document['id']] += 1
    ...
    def remove_document_from_index(self, document_id: str):
        ...
        for star in document[Indexes.STARS.value]:
            for term in star.split():
                self.index[Indexes.STARS.value][term][document_id] -= 1
                if self.index[Indexes.STARS.value][term][document_id] == 0:
                    self.index[Indexes.STARS.value][term].pop(document_id)
        for genre in document[Indexes.GENRES.value]:
            self.index[Indexes.GENRES.value][genre][document_id] -= 1
            if self.index[Indexes.GENRES.value][genre][document_id] == 0:
                self.index[Indexes.GENRES.value][genre].pop(document_id)
        for summary in document[Indexes.SUMMARIES.value]:
            for term in summary.split():
                self.index[Indexes.SUMMARIES.value][term][document_id] -= 1
                if self.index[Indexes.SUMMARIES.value][term][document_id] == 0:
                    self.index[Indexes.SUMMARIES.value][term].pop(document_id)
    ...
    def store_index(self, path: str, index_name: str = None):
        ...
        file = open(path + f'/{index_name}_indices.json', 'w')
    ...
    def load_index(self, path: str, index_name: str = None):
        ...
        index_name: str
            name of the index we want to load
        """
        try:
            if index_name is None:
                file = open(path + '/indices.json', 'r')
                self.index = json.load(file)
            else:
                file = open(path + f'/{index_name}_indices.json', 'r')
...
index = Index([])
for i in range(101, 9997):
    index.add_document_to_index({'id': str(i), 'stars': ['tom hanks', 'tom holland'], 'genres': ['crime'], 'summaries': ['funny']})
index.add_document_to_index({'id': '9997', 'stars': ['reza', 'timmy'], 'genres': ['drama', 'crime'], 'summaries': ['not that bad']})
index.add_document_to_index({'id': '9998', 'stars': ['arron', 'ali', 'mohammad'], 'genres': ['biography'], 'summaries': ['good']})
index.add_document_to_index({'id': '9999', 'stars': ['tom', 'mahdi', 'henry'], 'genres': ['drama', 'scipy'], 'summaries': ['so good']})
index.add_document_to_index({'id': '10000', 'stars': ['mohammad', 'tom'], 'genres': ['horror'], 'summaries': ['worst movie ever']})
...
print(index.check_if_index_loaded_correctly(None, index.load_index('Index')))
print(index.check_if_index_loaded_correctly(Indexes.DOCUMENTS.value, index.load_index('Index', Indexes.DOCUMENTS.value)))
print(index.check_if_index_loaded_correctly(Indexes.STARS.value, index.load_index('Index', Indexes.STARS.value)))
print(index.check_if_index_loaded_correctly(Indexes.GENRES.value, index.load_index('Index', Indexes.GENRES.value)))
print(index.check_if_index_loaded_correctly(Indexes.SUMMARIES.value, index.load_index('Index', Indexes.SUMMARIES.value)))
