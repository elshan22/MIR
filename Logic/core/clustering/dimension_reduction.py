...
class DimensionReduction:
    ...
    def __init__(self):
        self.pca = PCA()
        self.tsne_2d = TSNE(2)
        wandb.login(key='d54d4cf1c8c815e19c5a833e5f32576b51ebd67f', relogin=True)
    ...
    def wandb_plot_2d_tsne(self, data, project_name, run_name):
        ...
        embeddings_2d = self.convert_to_2d_tsne(data)
        plt.figure(figsize=(10, 8))
        plt.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1])
        plt.grid(True)
        wandb.log({"t-SNE Embedding": plt})
        plt.close()
    ...
    def wandb_plot_explained_variance_by_components(self, data, project_name, run_name):
        ...
        plt.grid(True)
        wandb.log({"Explained Variance": plt})
        plt.close()
